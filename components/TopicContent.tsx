...
import { getTopicExplanation, getTopicAncillaryData, MCQ, generateSpeech, FullTopicData } from '../services/geminiService';
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <button onClick={handleOpenGemini} className="w-full bg-[var(--bg-accent)] hover:bg-[var(--bg-accent-hover)] text-[var(--text-on-accent)] font-bold py-2 px-4 rounded-lg transition-colors">
                    {geminiButtonText}
                </button>
                <div>
                    <button onClick={handleSendToWhatsApp} className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded-lg transition-colors">
        ...
        <div className="bg-[var(--bg-secondary-alpha)] p-4 rounded-lg">
            ...
            <div className={`transition-opacity duration-500 ${isReady ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                ...
                <details className="mt-4 text-sm">
                    ...
                    <div className="mt-2 p-3 bg-[var(--bg-primary)] rounded-md text-[var(--text-secondary)] max-h-40 overflow-y-auto custom-scrollbar">
                        <p>{script}</p>
                    </div>
                </details>
            </div>
...
const Quiz = ({ mcqs }: { mcqs: MCQ[] }) => {
                    ...
                    {mcqs.map((mcq, index) => {
                        ...
                        return (
                            <div key={index} className={`p-4 rounded-lg ${isCorrect ? 'bg-[var(--bg-success)]' : 'bg-[var(--bg-danger)]'}`}>
                                <p className="font-semibold mb-2">{index + 1}. <ContentRenderer content={mcq.question} inline /></p>
    ...
    return (
      <div className="animate-fade-in">
        ...
        <p className="text-lg mb-6"><ContentRenderer content={currentQuestion.question} inline /></p>
        <div className="space-y-3">
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              ...
              >
                 <ContentRenderer content={option} inline />
              </button>
            );
          })}
        </div>
...
const SkeletonLoader = () => (
    <div className="animate-pulse">
        <div className="h-8 w-3/4 bg-[var(--bg-tertiary)] rounded mb-6"></div>
        <div className="flex space-x-4 border-b border-[var(--border-primary)] -mb-px">
            <div className="h-9 w-24 bg-[var(--bg-tertiary)] rounded-t-md"></div>
            <div className="h-9 w-20 bg-[var(--bg-tertiary)] rounded-t-md"></div>
            <div className="h-9 w-28 bg-[var(--bg-tertiary)] rounded-t-md"></div>
            <div className="h-9 w-32 bg-[var(--bg-tertiary)] rounded-t-md"></div>
        </div>
        <div className="mt-8 space-y-3">
            <div className="h-4 bg-[var(--bg-tertiary)] rounded w-full"></div>
            <div className="h-4 bg-[var(--bg-tertiary)] rounded w-full"></div>
            <div className="h-4 bg-[var(--bg-tertiary)] rounded w-5/6"></div>
            <div className="h-4 bg-[var(--bg-tertiary)] rounded w-full mt-6"></div>
...
const TopicContent = ({ topic, chapter, standard, subject }: { topic: string; chapter: string | null; standard: string | null; subject: string | null }) => {
    const [data, setData] = useState<Partial<FullTopicData> | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isAncillaryLoading, setIsAncillaryLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        getTopicExplanation(topic, chapter, standard, subject)
          .then(explanationText => {
            if (!isMounted) return;
            setData({ explanation: explanationText });
            setIsLoading(false); // Main content is ready
  
            // Fetch ancillary data in the background
            getTopicAncillaryData(topic, chapter, standard, subject)
              .then(ancillary => {
                  if (!isMounted) return;
                  setData(prevData => ({ ...prevData, ...ancillary }));
                  // Pre-fetch audio now that we have the script
                  if (ancillary.audioSummaryScript) {
                      generateSpeech(ancillary.audioSummaryScript, 'Kore')
                          .then(audio => isMounted && setPrefetchedAudio(audio))
                          .catch(err => {
              ...
              .catch(err => {
                  console.error("Ancillary data failed to load:", err.message);
                  // Optionally set an error state for ancillary data
              })
              .finally(() => {
                  if (isMounted) setIsAncillaryLoading(false);
              });
          })
          .catch((e: any) => {
            if (isMounted) {
    ...
    const renderContent = () => {
      switch (activeTab) {
        case 'explanation':
          return data.explanation ? <ExplanationCard content={data.explanation} /> : null;
        case 'quiz':
          return data.mcqs ? <Quiz mcqs={data.mcqs} /> : null;
        case 'visualize':
          return data.visualPrompt ? <VisualizationCard prompt={data.visualPrompt} /> : null;
        case 'audio':
           return <AudioPlayer script={data.audioSummaryScript ?? null} prefetchedAudio={prefetchedAudio} />;
        default:
          return null;
      }
    };
    ...
    const tabs = [
        { id: 'explanation', label: 'Explanation', disabled: false },
        { id: 'quiz', label: 'Quiz', disabled: isAncillaryLoading },
        { id: 'visualize', label: 'Visualize', disabled: isAncillaryLoading },
        { id: 'audio', label: 'Audio Summary', disabled: isAncillaryLoading },
    ];
