...
const SourceYearTooltip = ({ year }: { year?: string }) => {
    ...
    return (
        <div className="tooltip-container">
            <svg xmlns="http://www.w.org/2000/svg" className="h-4 w-4 text-[var(--text-muted)]" viewBox="0 0 20 20" fill="currentColor">
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0 mb-4 flex justify-between items-center">
                ...
                >
                    {showAnswers ? 'Hide Answers & Reset' : 'Show Answers'}
                </button>
            </div>
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        {testData.mcqs.map((mcq, index) => (
                            <div key={`mcq-${index}`} className="bg-[var(--bg-secondary)] p-4 rounded-lg">
                                <div className="flex justify-between items-start mb-2">
                                    <p className="font-semibold flex-1 flex items-center">
                                        {index + 1}.&nbsp;<ContentRenderer content={mcq.question} inline />
                                ...
                                <div className="space-y-2 mt-3">
                                    {mcq.options.map((option, optIndex) => {
                                        ...
                                        let buttonClass = "bg-[var(--bg-tertiary)] hover:bg-[var(--bg-quaternary)] border-transparent";

                                        if (showAnswers) {
                                            if (isCorrect) {
                                                buttonClass = "bg-[var(--bg-success)] border-[var(--border-success)] text-green-900 font-semibold";
                                            } else if (isSelected) {
                                                buttonClass = "bg-[var(--bg-danger)] border-[var(--border-danger)] text-red-900";
                                            } else {
                                                buttonClass = "bg-[var(--bg-secondary)] opacity-70 border-transparent";
                                            }
                                        } else if (isSelected) {
                                            buttonClass = "bg-[var(--bg-accent-subtle)] border-[var(--border-accent)] ring-1 ring-[var(--ring-accent)]";
                                        }
                                        ...
                                        return (
                                            <button
                                                ...
                                                onClick={() => setSelectedMcqAnswers(prev => ({ ...prev, [index]: option }))}
                                                className={`w-full text-left p-3 rounded-lg border transition-all duration-200 ${buttonClass}`}
                                            >
                                ...
                                {showAnswers && (
                                    <div className="mt-4 p-3 bg-[var(--bg-primary)] border-t border-[var(--border-primary)]">
                                        <p className="text-sm text-[var(--text-secondary)]"><ContentRenderer content={mcq.explanation} /></p>
                                    </div>
                                )}
                            </div>
                        ))}
                ...
                <section>
                    ...
                    <div className="space-y-4">
                         {testData.fillInTheBlanks.map((fib, index) => (
                            <div key={`fib-${index}`} className="bg-[var(--bg-secondary)] p-4 rounded-lg">
                                <div className="flex justify-between items-start mb-2">
                                    <p className="font-semibold flex-1 flex items-center">
                                        {index + 1}.&nbsp;<ContentRenderer content={fib.question} inline />
                                        <SourceYearTooltip year={fib.sourceYear} />
                                    </p>
                                </div>
                                {showAnswers && (
                                    <div className="mt-2 text-sm text-[var(--text-success)] font-mono p-2 bg-green-900/20 rounded-md">
                                       <strong>Answer:</strong>&nbsp;<ContentRenderer content={fib.answer} inline />
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <section key={section.title}>
                        ...
                        <div className="space-y-4">
                            {section.questions.map((q, index) => (
                                <div key={`${section.title}-${index}`} className="bg-[var(--bg-secondary)] p-4 rounded-lg">
                                    <div className="flex justify-between items-start mb-2">
                                         <p className="font-semibold flex-1 flex items-center">
                                            {index + 1}.&nbsp;<ContentRenderer content={q.question} inline />
                                            <SourceYearTooltip year={q.sourceYear} />
                                        </p>
                                    </div>
                                    {showAnswers && (
